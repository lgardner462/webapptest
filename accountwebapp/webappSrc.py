...
#######################################################################################################
		...
		if keyChoice == "generate":
			...
			os.system("ssh-keygen -N " + pass2 + " -t rsa -f " + keyPath + " -C " + email)
    		# convert private key to PuTTy ppk format via Expect script
                        os.system("/home/lgardner/git/accountwebapp/accountwebapp/convert_to_ppk.sh " + keyPath + " " + puttyKeyPath + " " + pass2)
                        print(keyPath)
                        print(puttyKeyPath)
...
def download_key():
	...
	keyZipPath = request.forms.get('keyPath')
...
def ack_account():
	...
	for user in tempList:
		...
		#Check if groups2.txt exists, if it exists populate oldSecondaryGroupList with groups, else create groups2.txt
		try:
			with  open(userSecondaryGroupFilePath, 'r') as f:
				for group in f.readlines():
					oldSecondaryGroupList.append(group)
					f.close()
		except IOError:
		...
		#Compare old group list from txt file to new group list from POST checkboxes, returns 0 if equal			
		if cmp(oldSecondaryGroupList,newSecondaryGroupList) != 0:			
			...
			f = open(userSecondaryGroupFilePath, 'w')
			for i in newSecondaryGroupList:
				f.write(i + "\n")
			f.close()
...
###################### determine action on each user: ACK, DENY, none
	...
	return template('ack_post_account', chgroupDict=chgroupDict, ackList=ackList, nackList=nackList,chgroupList=chgroupList,listOfGroupLists=listOfGroupLists)
